...
from src.presentation.ui.dashboard_manager import DashboardManager
...
export default RAGDashboardEmbed;"""
    ...
    def _validate_embed_access(self, embed_config: EmbedConfig, api_key: Optional[str],
        ...
        return not (embed_config.allowed_domains and domain and domain not in embed_config.allowed_domains)
        ...
        if not embeds:
            ...
            for embed_config in embeds:
                with st.expander(f"🔗 {embed_config.name}", expanded=False):
                    ...
                    with col2:
                        if st.button("📋 Get Code", key=f"code_{embed_config.id}"):
                        ...
                        if st.button("🗑️ Delete", key=f"delete_{embed_config.id}"):
