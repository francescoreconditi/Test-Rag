...
# Initialize router
router = APIRouter(prefix="/audio", tags=["Overview Audio"])
...
async def get_audio_service_status():
    ...
    return {"service_available": True, "dependencies": deps, "cache_dir": str(audio_service.cache_dir)}
...
async def generate_audio_overview(request: AudioOverviewRequest, background_tasks: BackgroundTasks):
    ...
    if not deps["any_available"]:
        raise HTTPException(status_code=503, detail="No TTS engine available. Install edge-tts or gtts.")
    ...
    try:
        if request.use_rag and rag_engine:
            ...
            result = await audio_service.generate_audio_from_dialogue(
                dialogue_turns=dialogue_turns, language=request.language
            )
        ...
        if result["success"]:
            ...
            background_tasks.add_task(cleanup_old_files, audio_service, max_age_hours=2)
            ...
            raise HTTPException(status_code=500, detail=f"Audio generation failed: {result.get('error')}")
...
async def download_audio(audio_id: str):
    ...
    if not audio_file:
        for cached_file in audio_service.cache_dir.glob("cached_*.mp3"):
    ...
    return FileResponse(path=str(audio_file), media_type="audio/mpeg", filename=f"audio_overview_{audio_id}.mp3")
...
async def stream_audio(audio_id: str, response: Response):
    ...
    return FileResponse(path=str(audio_file), media_type="audio/mpeg")
...
async def clean_audio_cache(max_age_hours: int = 24):
    ...
    return {"success": True, "files_cleaned": cleaned, "max_age_hours": max_age_hours}
