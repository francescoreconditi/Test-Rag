...
from src.application.services.hybrid_retrieval import HybridRetrievalService
    ...
    for record in data.get('mapped_records', []):
        try:
            ...
            if 'normalized_value' in record:
                ...
                if 'margin' in metric.lower() or 'ros' in metric.lower():
...
async def process_documents_flow(
    ...
    for file_path in file_paths:
        try:
            ...
            if validate:
                validated_data = validate_financial_data(mapped_data)
            else:
                validated_data = mapped_data
