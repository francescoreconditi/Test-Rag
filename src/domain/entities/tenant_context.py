...
class TenantUsageStats:
    ...
    def is_within_limits(self, limits: TenantResourceLimits) -> bool:
        ...
        if limits.max_storage_gb > 0 and self.storage_used_gb >= limits.max_storage_gb:
            return False
        if limits.max_concurrent_users > 0 and self.current_concurrent_users >= limits.max_concurrent_users:
            return False
        return True
...
class TenantContext:
    ...
    @classmethod
    def create_new_tenant(cls, 
                         tenant_name: str,
                         organization: str,
                         admin_email: str,
                         tier: TenantTier = TenantTier.BASIC) -> 'TenantContext':
    ...
    def can_perform_action(self, action: str) -> bool:
        ...
        action_checks = {
            "upload_document": self.resource_limits.max_documents_per_month < 0 or 
                             self.current_usage.documents_this_month < self.resource_limits.max_documents_per_month,
            "execute_query": self.resource_limits.max_queries_per_day < 0 or 
                           self.current_usage.queries_today < self.resource_limits.max_queries_per_day,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_usage': {
                ...
                'last_activity': self.current_usage.last_activity.isoformat() if self.current_usage.last_activity else None
            },
