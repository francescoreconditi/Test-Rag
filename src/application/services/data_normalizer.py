...
class DataNormalizer:
    ...
    def _calculate_confidence(self, original: str, cleaned: str) -> float:
        ...
        len(cleaned) / len(original) if len(original) > 0 else 0
    ...
    def _is_percentage(self, text: str) -> bool:
        ...
        return any(re.search(pattern, text) for pattern in self.percentage_patterns)

    def _load_cached_rates(self):
        """Load cached currency rates."""
        try:
            if self.rates_cache_file.exists():
                with open(self.rates_cache_file) as f:
                    data = json.load(f)
    ...
    def get_normalization_summary(self, results: dict[str, NormalizedValue]) -> dict[str, Any]:
        ...
        currencies = {r.currency for r in results.values() if r.currency}
        scales = {r.scale_applied for r in results.values()}
