...
from src.application.services.report_scheduler import (
    ReportScheduler, ScheduledReport, ReportType, ReportFormat, ReportFrequency, ReportDelivery
)
...
def render_analytics_api(tenant_context):
    ...
    with col1:
        ...
        if test_endpoint == "POST /query":
            ...
            metrics = st.multiselect("Metrics", ["queries", "documents", "users"], default=["queries"])
...
def render_export_templates(report_scheduler: ReportScheduler, tenant_context):
    ...
    for i, template in enumerate(templates):
        with st.expander(f"📋 {template['name']}", expanded=i == 0):
            ...
            with col3:
                ...
                if st.button(f"✏️ Customize", key=f"customize_{i}"):
    ...
    with st.form("custom_template"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            include_header = st.checkbox("Include header/logo", value=True)
            include_charts = st.checkbox("Include charts", value=True)
            include_tables = st.checkbox("Include data tables", value=True)
        
        with col2:
            include_footer = st.checkbox("Include footer", value=True)
            include_metrics = st.checkbox("Include KPI metrics", value=True)
            include_insights = st.checkbox("Include AI insights", value=False)
