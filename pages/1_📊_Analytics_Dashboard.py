...
def main():
    ...
    industry = st.sidebar.selectbox("Industry Benchmark", ["manufacturing", "services", "retail"], index=0)
    ...
    with st.spinner("Generating analytics..."):
        dashboard_data = analytics_service.generate_dashboard_data(sample_data, sample_periods, industry)
    ...
    with col2:
        ...
        fig_health = go.Figure(
            go.Indicator(
                ...
                domain={"x": [0, 1], "y": [0, 1]},
                title={
                    "text": f"Financial Health Score<br><span style='font-size:0.8em;color:gray'>{health_data['level']}</span>"
                },
    ...
    for i, (metric, label, icon) in enumerate(metric_configs):
        with cols[i]:
            if metric in key_metrics:
                ...
                st.metric(label=f"{icon} {label}", value=value, delta=delta_value, delta_color=delta_color)
    ...
    for i, kpi in enumerate(kpis[:8]):  # Show first 8 KPIs
        ...
        with kpi_cols[col_idx]:
            if kpi["actual_value"] is not None:
                ...
                status_colors = {"green": "#28a745", "yellow": "#ffc107", "red": "#dc3545", "gray": "#6c757d"}
                        ...
                        go.Indicator(
                            ...
                            gauge={
                                ...
                                "threshold": {
                                    ...
                                    "thickness": 0.75,
                                    "value": ebitda_kpi.get("target_value", 15),
                                },
                            },
                        )
                    )
                        ...
                        go.Indicator(
                            ...
                            gauge={
                                ...
                                "threshold": {
                                    "line": {"color": "red", "width": 4},
                                    "thickness": 0.75,
                                    "value": dso_kpi.get("target_value", 45),
                                },
                            },
                        )
                    )
    ...
    with chart_tabs[1]:
        ...
        fig_waterfall = go.Figure(
            go.Waterfall(
                ...
                text=[
                    f"€{ricavi / 1e6:.1f}M",
                    f"-€{cogs / 1e6:.1f}M",
                    f"-€{(ebitda - ebit) / 1e6:.1f}M",
                    f"-€{(ebit - utile_netto) / 1e6:.1f}M",
        ...
        fig_waterfall.update_layout(title="Profitability Waterfall (€ Millions)", height=400)
    ...
    with chart_tabs[2]:
        ...
        if sample_periods:
            ...
            fig_trends.update_layout(
                title="Financial Trends (€ Millions)", xaxis_title="Period", yaxis_title="Amount (€M)", height=400
            )
