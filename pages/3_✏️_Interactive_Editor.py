...
def display_metric_editor(session_id, metric_name, metric_data, editor_service):
    ...
    with col2:
        ...
        if metric_data.get("editable", True) and not metric_data.get("calculated", False):
            ...
            # Update button
            if st.button("💾", key=f"save_{metric_name}", help="Save changes"):
                if new_value != current_value:
                    with st.spinner("Updating..."):
                        result = editor_service.update_metric_value(
                            session_id, metric_name, new_value, user_comment="Updated via interactive editor"
                        )
                        ...
                        if result["success"]:
                            ...
                            if result.get("requires_override"):
                                if st.button("Force Update", key=f"force_{metric_name}"):
                                    result = editor_service.update_metric_value(
                                        session_id, metric_name, new_value, validation_override=True
                                    )
                                    if result["success"]:
            ...
            if current_value is not None:
                formatted_value = (
                    f"{current_value:,.2f}" if isinstance(current_value, (int, float)) else str(current_value)
                )
                st.text_input("Value", value=formatted_value, disabled=True, key=f"readonly_{metric_name}")
                unit_display = metric_data.get("unit", "")
            else:
                st.text_input("Value", value="Not available", disabled=True, key=f"readonly_{metric_name}")
                unit_display = ""
    ...
    with col4:
        ...
        status_colors = {"valid": "🟢", "warning": "🟡", "error": "🔴", "pending": "⚪"}
...
def display_edit_history(history):
    ...
    if history_data:
        ...
        with st.expander("🔄 Undo Operations"):
            operation_options = [
                (op["ID"], f"{op['Timestamp']} - {op['Operation']} {op['Metric']}") for op in history_data
            ]

            if operation_options:
                st.selectbox("Select operation to undo:", operation_options, format_func=lambda x: x[1])
...
def main():
    ...
    if st.sidebar.button("🆕 Start New Session"):
        session_id = editor_service.start_editing_session(document_path="sample_document.pdf", user_id="user_001")
    ...
    with tabs[2]:
        ...
        with col2:
            ...
            source_ref = st.text_input(
                "Source reference (optional)", placeholder="e.g., document.pdf|p.5|row:New Metric"
            )
    ...
    with col1:
        ...
        editable_count = sum(1 for m in editable_data.get("data", {}).values() if m.get("editable", True))
