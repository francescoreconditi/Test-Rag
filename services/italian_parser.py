...
class ItalianNumberParser:
    ...
    def detect_document_metadata(self, text: str) -> dict[str, Any]:
        ...
        # Rileva periodo
        for _period_type, pattern in self.period_patterns.items():
...
class FinancialValidator:
    ...
    def validate_balance_sheet(self, values: list[ProvenancedValue]) -> list[str]:
        ...
        if attivo and passivo and abs(attivo - passivo) / max(abs(attivo), abs(passivo)) > self.tolerance:
    ...
    def validate_ranges(self, values: list[ProvenancedValue]) -> list[str]:
        ...
        for value in values:
            # Test percentuali
            if value.unit == '%' and (value.value < -1.0 or value.value > 1.0):
