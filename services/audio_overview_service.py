...
class AudioOverviewService:
    ...
    def _generate_template_dialogue(
        ...
        if language == "it":
            ...
            dialogue.append(("host1", f"Benvenuti! Oggi parliamo di una domanda interessante{': ' + query if query else '.'}"))
            ...
            if metadata and metadata.get('num_sources'):
                dialogue.append(("host2", f"Queste informazioni provengono da {metadata['num_sources']} fonti diverse."))
            
            dialogue.append(("host1", "In conclusione, abbiamo visto aspetti molto importanti di questo argomento."))
            dialogue.append(("host2", "Sì, spero che questa discussione sia stata utile per comprendere meglio il tema."))
            
        else:
            # English dialogue template
            dialogue.append(("host1", f"Welcome! Today we're discussing an interesting question{': ' + query if query else '.'}"))
            ...
            dialogue.append(("host2", "Yes, I hope this discussion has been helpful in understanding the subject better."))
...
"""
    ...
    async def process_rag_response(
        ...
        # Generate audio
        result = await self.generate_audio_from_dialogue(
